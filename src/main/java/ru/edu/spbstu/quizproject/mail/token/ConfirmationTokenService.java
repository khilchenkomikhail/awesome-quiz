...
import ru.edu.spbstu.quizproject.mail.EmailSender;
import ru.edu.spbstu.quizproject.service.UserService;
...
public class ConfirmationTokenService {
    ...
    private static final String confirmationLink = "http://localhost:8080/api/v1/registration/confirm-account?token=";

    public void sendConfirmationToken(User user) {
        ConfirmationToken token = createConfirmationToken(user);
        emailSender.send(
                user.getEmail(),
                buildEmail(user.getUsername(), confirmationLink + token.getToken())
        );
    }
